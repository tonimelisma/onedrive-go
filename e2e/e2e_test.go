...
func newFileOpsFixture(t *testing.T, name string) (string, string) {
	t.Helper()

	cfgPath := writeMinimalConfig(t)
	testFolder := fmt.Sprintf("onedrive-go-e2e-%s-%d", name, time.Now().UnixNano())
	t.Cleanup(func() {
		cleanupRemoteFolder(t, testFolder)
	})

	return cfgPath, testFolder
}

func writeUploadFixtureFile(t *testing.T, content []byte) string {
	t.Helper()
...
func mkdirFixturePath(t *testing.T, cfgPath string, remotePath string) {
	t.Helper()

	_, stderr := runCLIWithConfig(t, cfgPath, nil, "mkdir", "/"+remotePath)
	assert.Contains(t, stderr, "Created")
	waitForRemoteFixtureSeedVisible(t, cfgPath, nil, drive, "/"+remotePath)
}

func putFixtureFile(t *testing.T, cfgPath string, remotePath string, content []byte) {
	t.Helper()

	localPath := writeUploadFixtureFile(t, content)
	_, stderr := runCLIWithConfig(t, cfgPath, nil, "put", localPath, "/"+remotePath)
	assert.Contains(t, stderr, "Uploaded")
	waitForRemoteFixtureSeedVisible(t, cfgPath, nil, drive, "/"+remotePath)
}

func TestE2E_FileOps_Whoami(t *testing.T) {
	registerLogDump(t)
	...
	textStdout, _ := pollCLIWithConfigRetryingTransientGraphFailures(
		t, cfgPath, nil, drive, transientGraphRetryTimeout, "whoami",
	)

	email := strings.SplitN(drive, ":", 2)[1]
	assert.Contains(t, textStdout, email, "whoami text output should contain the account email")
}

func TestE2E_FileOps_LsRoot(t *testing.T) {
	registerLogDump(t)
...
func TestE2E_FileOps_Mkdir(t *testing.T) {
	registerLogDump(t)

	cfgPath, testFolder := newFileOpsFixture(t, "mkdir")
	testSubfolder := testFolder + "/subfolder"

	mkdirFixturePath(t, cfgPath, testSubfolder)
	stdout, _ := waitForRemoteParentListingContains(t, cfgPath, nil, drive, "/"+testFolder, "subfolder")
	assert.Contains(t, stdout, "subfolder")
}

func TestE2E_FileOps_Put(t *testing.T) {
	registerLogDump(t)

	cfgPath, testFolder := newFileOpsFixture(t, "put")
	testFile := testFolder + "/test.txt"
	testContent := []byte("Hello from onedrive-go E2E test!\n")

	mkdirFixturePath(t, cfgPath, testFolder)
	putFixtureFile(t, cfgPath, testFile, testContent)
...
func TestE2E_FileOps_LsFolder(t *testing.T) {
	registerLogDump(t)

	cfgPath, testFolder := newFileOpsFixture(t, "ls-folder")
	testSubfolder := testFolder + "/subfolder"
	testFile := testFolder + "/test.txt"
	testContent := []byte("Hello from onedrive-go E2E test!\n")
...
func TestE2E_FileOps_Stat(t *testing.T) {
	registerLogDump(t)

	cfgPath, testFolder := newFileOpsFixture(t, "stat")
	testFile := testFolder + "/test.txt"
	testContent := []byte("Hello from onedrive-go E2E test!\n")
...
func TestE2E_FileOps_Get(t *testing.T) {
	registerLogDump(t)

	cfgPath, testFolder := newFileOpsFixture(t, "get")
	testFile := testFolder + "/test.txt"
	testContent := []byte("Hello from onedrive-go E2E test!\n")
...
func TestE2E_FileOps_RmFile(t *testing.T) {
	registerLogDump(t)

	cfgPath, testFolder := newFileOpsFixture(t, "rm-file")
	testFile := testFolder + "/test.txt"
...
func TestE2E_FileOps_RmSubfolder(t *testing.T) {
	registerLogDump(t)

	cfgPath, testFolder := newFileOpsFixture(t, "rm-subfolder")
	testSubfolder := testFolder + "/subfolder"
...
func TestE2E_FileOps_RmPermanent(t *testing.T) {
	registerLogDump(t)

	cfgPath, testFolder := newFileOpsFixture(t, "rm-permanent")
...
func TestE2E_FileOps_Status(t *testing.T) {
	registerLogDump(t)
