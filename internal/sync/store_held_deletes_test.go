...
func TestSyncStore_HeldDeleteConsumeRequiresMatchingItemID(t *testing.T) {
	...
	require.NoError(t, store.ConsumeHeldDelete(ctx, driveID, ActionRemoteDelete, "reuse.txt", "other-item"))
	approved, err := store.ListHeldDeletesByState(ctx, HeldDeleteStateApproved)
	require.NoError(t, err)
	require.Len(t, approved, 2)

	require.NoError(t, store.ConsumeHeldDelete(ctx, driveID, ActionRemoteDelete, "reuse.txt", "old-item"))
	approved, err = store.ListHeldDeletesByState(ctx, HeldDeleteStateApproved)
	require.NoError(t, err)
...
func TestSyncStore_DeleteHeldDeleteRequiresMatchingItemID(t *testing.T) {
	...
	require.NoError(t, store.DeleteHeldDelete(ctx, driveID, ActionRemoteDelete, "delete-me.txt", "other-item"))
	held, err := store.ListHeldDeletesByState(ctx, HeldDeleteStateHeld)
	require.NoError(t, err)
	require.Len(t, held, 1)

	require.NoError(t, store.DeleteHeldDelete(ctx, driveID, ActionRemoteDelete, "delete-me.txt", "item-1"))
	held, err = store.ListHeldDeletesByState(ctx, HeldDeleteStateHeld)
	require.NoError(t, err)
	assert.Empty(t, held)
}
