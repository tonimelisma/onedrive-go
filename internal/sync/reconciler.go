...
func newFolderState(item *Item) folderState {
	return folderState{
		...
		// A folder "exists remotely" only if it has a server-assigned ItemID.
		// Scanner-generated IDs (prefixed "local:") indicate the folder is
		// local-only and needs to be created remotely (D5), not adopted (D2).
		remoteExists: !item.IsDeleted && item.ItemID != "" && !isLocalItemID(item.ItemID),
		synced:       isSynced(item),
		tombstoned:   item.IsDeleted,
	}
}

// isLocalItemID returns true if the ItemID was generated by the local scanner
// (prefixed with "local:") rather than assigned by the Graph API server.
func isLocalItemID(id string) bool {
