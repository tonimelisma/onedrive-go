...
func TestRecordFailure_NormalizesDirectionFromActionType(t *testing.T) {
	...
	require.NoError(t, err)
	require.Len(t, issues, 1)
	assert.Equal(t, ActionFolderCreate, issues[0].ActionType)
	assert.Equal(t, DirectionDownload, issues[0].Direction)
}
...
func TestCommitMutation_UploadSuccess_DoesNotClearSyncFailures(t *testing.T) {
	mgr, _ := newTestSyncStoreForFailures(t)
	ctx := context.Background()
	...
	// D-6: CommitMutation does NOT clear sync_failures — that's the engine's
	// responsibility via clearFailureOnSuccess.
	issues, err := mgr.ListSyncFailures(ctx)
	require.NoError(t, err)
	assert.Len(t, issues, 1, "sync_failures should survive CommitMutation")
}

func TestCommitMutation_UploadSuccess_NoIssue_NoError(t *testing.T) {
	mgr, _ := newTestSyncStoreForFailures(t)
	ctx := context.Background()
...
func TestCommitMutation_DownloadSuccess_DoesNotClearSyncFailures(t *testing.T) {
	mgr, _ := newTestSyncStoreForFailures(t)
	ctx := context.Background()
	...
	// D-6: CommitMutation does NOT clear sync_failures — engine owns that.
	issues, err := mgr.ListSyncFailures(ctx)
	require.NoError(t, err)
	assert.Len(t, issues, 1, "sync_failures should survive CommitMutation")
}
...
func TestUpsertActionableFailures(t *testing.T) {
	...
	failures := []ActionableFailure{
		{Path: "CON.txt", DriveID: driveid.ID{}, Direction: DirectionUpload, IssueType: IssueInvalidFilename, Error: "reserved name"},
		{Path: "long/path.txt", DriveID: driveid.ID{}, Direction: DirectionUpload, IssueType: IssuePathTooLong, Error: "too long"},
	}
	...
	for _, row := range issues {
		assert.Equal(t, CategoryActionable, row.Category)
	}
}
...
func TestUpsertActionableFailures_UpdateExisting(t *testing.T) {
	...
	failures := []ActionableFailure{
		{Path: "CON.txt", DriveID: driveid.ID{}, Direction: DirectionUpload, IssueType: IssueInvalidFilename, Error: "reserved name v1"},
	}
	err := mgr.UpsertActionableFailures(ctx, failures)
	require.NoError(t, err)
...
func TestClearResolvedActionableFailures(t *testing.T) {
	...
	failures := []ActionableFailure{
		{Path: "CON.txt", DriveID: driveid.ID{}, Direction: DirectionUpload, IssueType: IssueInvalidFilename, Error: "reserved"},
		{Path: "NUL.txt", DriveID: driveid.ID{}, Direction: DirectionUpload, IssueType: IssueInvalidFilename, Error: "reserved"},
		{Path: "PRN.txt", DriveID: driveid.ID{}, Direction: DirectionUpload, IssueType: IssueInvalidFilename, Error: "reserved"},
	}
	err := mgr.UpsertActionableFailures(ctx, failures)
	require.NoError(t, err)
