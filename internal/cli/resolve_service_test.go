...
type stubResolveConflictQueueStore struct {
	...
	allConflicts     []syncengine.ConflictRecord
	requestResult    syncengine.ConflictRequestResult
	requestErr       error
	requestCalls     int
	requestedIDs     []string
...
func (s *stubResolveConflictQueueStore) ListConflicts(context.Context) ([]syncengine.ConflictRecord, error) {
	return s.conflicts, nil
}

func (s *stubResolveConflictQueueStore) ListAllConflicts(context.Context) ([]syncengine.ConflictRecord, error) {
...
func TestResolveService_QueueEachConflictResolution_DryRun(t *testing.T) {
	...
	store := &stubResolveConflictQueueStore{
		conflicts: []syncengine.ConflictRecord{{ID: "id-1", Path: "/foo.txt"}},
	}

	require.NoError(t, queueEachConflictResolution(t.Context(), cc, store, store.conflicts, syncengine.ResolutionKeepLocal, true))
...
func TestResolveService_QueueSingleConflictResolution_AlreadyResolvedIsReplaySafe(t *testing.T) {
	...
	store := &stubResolveConflictQueueStore{
		allConflicts: []syncengine.ConflictRecord{
			{ID: "id-1", Path: "/foo.txt", Resolution: syncengine.ResolutionKeepBoth},
		},
	}

	require.NoError(t, queueSingleConflictResolution(t.Context(), cc, store, "/foo.txt", syncengine.ResolutionKeepLocal, false))
...
func TestResolveService_RequestConflictResolutionConcurrentCLIsLastWriteWinsWhileQueued(t *testing.T) {
	...
	statuses := make(chan syncengine.ConflictRequestStatus, requestCount)
	...
	for i := range requestCount {
		strategy := syncengine.ResolutionKeepLocal
		if i%2 == 1 {
			strategy = syncengine.ResolutionKeepRemote
		}
	...
	for status := range statuses {
		...
		case syncengine.ConflictRequestAlreadyApplying, syncengine.ConflictRequestAlreadyResolved:
	...
	assert.Contains(t, []string{syncengine.ResolutionKeepLocal, syncengine.ResolutionKeepRemote}, conflict.RequestedResolution)
}
