...
func ensureNoStaleArchitecturePhrases(repoRoot string) error {
	staleChecks := []staleCheck{
		...
		{name: "stale shortcut child drain acker interface", pattern: regexp.MustCompile(`\bshortcutChild` + `DrainAcker\b`)},
		{name: "stale shortcut child artifact acker interface", pattern: regexp.MustCompile(`\bshortcutChild` + `ArtifactCleanupAcker\b`)},
		{name: "stale shortcut child lifecycle acker interface", pattern: regexp.MustCompile(`\bshortcutChild` + `LifecycleAcker\b`)},
		{name: "stale shortcut child ack capability interface", pattern: regexp.MustCompile(`\bShortcutChildAck` + `Capability\b`)},
		{name: "stale shortcut parent lifecycle handle", pattern: regexp.MustCompile(`\bshortcutParent` + `LifecycleHandle\b`)},
