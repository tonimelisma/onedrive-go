...
func TestBuildStandaloneMountSpecs_PreservesOrderAndReportingFields(t *testing.T) {
	...
	assert.Equal(t, mountID(first.CanonicalID.String()), mounts[0].id())
...
// Validates: R-2.8.1
func TestBuildRuntimeWork_AddsChildProjectionAfterParent(t *testing.T) {
	...
	decisions, err := buildRuntimeWork(
		[]StandaloneMountConfig{parent},
		testParentSnapshots(&parent, testPublishedShortcutChild(t)),
		dataDir,
	)
	require.NoError(t, err)
	...
	assert.Equal(t, mountID(config.ChildMountID(parent.CanonicalID.String(), "binding-child-docs")), childMount.id())
	assert.Equal(t, parentMount.id(), childMount.childParentMountID())
...
// Validates: R-2.8.1
func TestBuildRuntimeWork_ParentPausePausesChildWithoutParentReservationSynthesis(t *testing.T) {
	t.Setenv("XDG_DATA_HOME", t.TempDir())
	parent := testStandaloneMount(t, "personal:owner@example.com", "Parent")
	parent.Paused = true
...
// Validates: R-2.8.1
func TestBuildRuntimeWork_ParentBlockedSnapshotHasNoChildWork(t *testing.T) {
	t.Setenv("XDG_DATA_HOME", t.TempDir())
	parent := testStandaloneMount(t, "personal:owner@example.com", "Parent")

	decisions, err := buildRuntimeWork(
		[]StandaloneMountConfig{parent},
		testParentSnapshots(&parent),
		t.TempDir(),
	)
	require.NoError(t, err)
...
// Validates: R-2.8.1
func TestBuildRuntimeWork_FinalDrainChildDoesNotSynthesizeParentReservation(t *testing.T) {
	t.Setenv("XDG_DATA_HOME", t.TempDir())
	parent := testStandaloneMount(t, "personal:owner@example.com", "Parent")
	child := testPublishedShortcutChild(t)
	child.Mode = syncengine.ShortcutChildRunModeFinalDrain
	...
	decisions, err := buildRuntimeWork(
		[]StandaloneMountConfig{parent},
		testParentSnapshots(&parent, child),
		t.TempDir(),
	)
	require.NoError(t, err)
	require.Len(t, decisions.Mounts, 2)
...
// Validates: R-2.8.1
func TestBuildRuntimeWork_UsesParentPublishedChildLocalRoot(t *testing.T) {
	t.Setenv("XDG_DATA_HOME", t.TempDir())
	parent := testStandaloneMount(t, "personal:owner@example.com", "Parent")
	child := testPublishedShortcutChild(t)
...
// Validates: R-2.8.1
func TestBuildRuntimeWork_MalformedChildDoesNotSynthesizeParentReservation(t *testing.T) {
	t.Setenv("XDG_DATA_HOME", t.TempDir())
	parent := testStandaloneMount(t, "personal:owner@example.com", "Parent")
	child := testPublishedShortcutChild(t)
...
// Validates: R-2.8.1
func TestBuildRuntimeWork_ChildDeltaCapabilityComesFromMountTokenOwner(t *testing.T) {
	t.Setenv("XDG_DATA_HOME", t.TempDir())
...
// Validates: R-2.8.1
func TestBuildRuntimeWork_StandaloneContentRootDoesNotSuppressChild(t *testing.T) {
	t.Setenv("XDG_DATA_HOME", t.TempDir())
	parent := testStandaloneMount(t, "business:owner@example.com", "Parent")
...
// Validates: R-2.8.1
func TestBuildRuntimeWork_MissingParentSkipsChild(t *testing.T) {
	t.Setenv("XDG_DATA_HOME", t.TempDir())
	parent := testStandaloneMount(t, "personal:owner@example.com", "Parent")
