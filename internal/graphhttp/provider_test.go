...
// Validates: R-6.2.10, R-6.8.8
func TestProvider_InteractiveForAccount_ReusesThrottleGatePerAccount(t *testing.T) {
	t.Parallel()

	provider := NewProvider(testLogger())

	first := provider.InteractiveForAccount("a@example.com")
	second := provider.InteractiveForAccount("a@example.com")
	third := provider.InteractiveForAccount("b@example.com")

	require.Same(t, first.Meta, second.Meta, "same account should reuse metadata client")
	...
	assert.Same(t, firstRetry.ThrottleGate, secondRetry.ThrottleGate,
		"same account should share throttle gate")
	assert.NotSame(t, firstRetry.ThrottleGate, thirdRetry.ThrottleGate,
		"different accounts should not share throttle gate")
...
// Validates: R-6.8.8
func TestProvider_InteractiveForAccount_TransportRetryDeadlineIsRetryable(t *testing.T) {
	t.Parallel()

	provider := NewProvider(testLogger())
	retryTransport, ok := provider.InteractiveForAccount("a@example.com").Meta.Transport.(*retry.RetryTransport)
	require.True(t, ok)
