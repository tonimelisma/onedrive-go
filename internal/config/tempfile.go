...
func chmodTrustedTempPath(path string, mode os.FileMode, desc string) error {
	...
	if err := os.Chmod(path, mode); err != nil { //nolint:gosec // Path is an os.CreateTemp-managed temp file in the config directory.
...
func renameTrustedTempPath(src, dst, desc string) error {
	...
	//nolint:gosec // Source and destination are managed atomic-write paths in the config directory.
