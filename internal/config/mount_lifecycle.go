...
func shortcutUnavailableLifecycleDetails() map[MountStateReason]MountLifecycleDetail {
	return map[MountStateReason]MountLifecycleDetail{
		MountStateReasonShortcutBindingUnavailable: {
			...
			RecoveryAction: "Restore target access or recreate the OneDrive shortcut alias if it no longer resolves.",
		},
		MountStateReasonLocalProjectionUnavailable: {
			...
			RecoveryAction: "Fix the filesystem permission, lock, or disk issue for the protected path, then rerun sync.",
		},
		MountStateReasonLocalAliasRenameUnavailable: {
			...
			RecoveryAction: "Fix auth, network, permission, or filesystem issues, or rename the shortcut alias directory back.",
		},
		MountStateReasonLocalAliasDeleteUnavailable: {
			...
			RecoveryAction: "Fix auth, network, or permission issues, or recreate the shortcut alias directory.",
		},
		MountStateReasonLocalRootUnavailable: {
			...
			RecoveryAction: "Restore the protected path or fix local filesystem permissions, then rerun sync.",
		},
	}
}
...
func shortcutConflictLifecycleDetails() map[MountStateReason]MountLifecycleDetail {
	return map[MountStateReason]MountLifecycleDetail{
		MountStateReasonDuplicateContentRoot: {
			...
			RecoveryAction: "Remove, move, or rename one duplicate OneDrive shortcut alias.",
		},
		MountStateReasonExplicitStandaloneContentRoot: {
			...
			RecoveryAction: "Remove the OneDrive shortcut alias or remove the configured standalone shared-folder drive.",
		},
		MountStateReasonLocalProjectionConflict: {
			...
			RecoveryAction: "Merge, move, or remove one listed child projection path, then rerun sync.",
		},
		MountStateReasonLocalAliasRenameConflict: {
			...
			RecoveryAction: "Keep exactly one listed same-folder renamed directory, or restore the original shortcut alias name, then rerun sync.",
		},
		MountStateReasonPathReservedByPendingRemoval: {
			...
			RecoveryAction: "Move or delete content in the old removed child projection so the new shortcut can start.",
		},
		MountStateReasonLocalRootCollision: {
			...
			StatusDetail: "The child mount path collides with a local file, symlink, or unsafe path. " +
				"The path remains reserved and will be rechecked.",
			RecoveryAction: "Remove or rename the listed blocking local object, then rerun sync.",
		},
	}
}
...
func shortcutPendingRemovalLifecycleDetails() map[MountStateReason]MountLifecycleDetail {
	return map[MountStateReason]MountLifecycleDetail{
		MountStateReasonShortcutRemoved: {
			...
			RecoveryAction: "No action is needed unless the child projection still contains local content.",
		},
		...
		MountStateReasonRemovedProjectionUnavailable: {
			...
			RecoveryAction: "Fix the filesystem, lock, disk, or child state-file issue for the protected path, then rerun sync.",
		},
	}
}
