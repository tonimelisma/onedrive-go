...
// Validates: R-2.10.47
func TestRecoverSyncStore_RepairsReadableStoreInPlace(t *testing.T) {
	t.Parallel()
	...
	result, err := RecoverSyncStore(ctx, dbPath, logger)
	require.NoError(t, err)
	assert.Equal(t, RecoverActionRepair, result.Action)
...
func TestRecoverSyncStore_RebuildPreservesDurableIntent(t *testing.T) {
	t.Parallel()
	...
	result, err := RecoverSyncStore(t.Context(), dbPath, newTestLogger(t))
	require.NoError(t, err)
	assert.Equal(t, RecoverActionRebuild, result.Action)
	...
	require.NoError(t, err)
	assert.Equal(t, synctypes.ResolutionUnresolved, conflict.Resolution)
...
// Validates: R-2.10.47
func TestRecoverSyncStore_ResetsWhenExistingDatabaseCannotBeSalvaged(t *testing.T) {
	t.Parallel()
	...
	result, err := RecoverSyncStore(t.Context(), dbPath, newTestLogger(t))
	require.NoError(t, err)
	assert.Equal(t, RecoverActionReset, result.Action)
	...
	heldDeletes, err := store.ListHeldDeletesByState(t.Context(), synctypes.HeldDeleteStateHeld)
	require.NoError(t, err)
