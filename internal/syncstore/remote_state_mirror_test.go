...
func TestCommitObservation_NewItemCreatesMirrorRowAndToken(t *testing.T) {
	...
	err := mgr.CommitObservation(ctx, []synctypes.ObservedItem{{
		DriveID:  driveID,
		ItemID:   "item1",
		ParentID: "root",
		Path:     "hello.txt",
...
func TestCommitObservation_FilteredRowStoresFilterMetadata(t *testing.T) {
	...
	err := mgr.CommitObservation(ctx, []synctypes.ObservedItem{{
		...
		Filtered:         true,
		FilterGeneration: 7,
		FilterReason:     synctypes.RemoteFilterPathScope,
	}}, "delta-token-1", driveID)
	require.NoError(t, err)
	...
	assert.Equal(t, synctypes.RemoteFilterPathScope, row.FilterReason)
}
