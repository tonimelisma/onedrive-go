...
func TestReadDetailedStatusSnapshot_ReadOnlyDB(t *testing.T) {
	...
	request, err := store.RequestConflictResolution(ctx, "c1", synctypes.ResolutionKeepLocal)
	require.NoError(t, err)
	...
	assert.Equal(t, synctypes.ResolutionKeepLocal, snapshot.Conflicts[0].RequestedResolution)
	assert.Equal(t, synctypes.ConflictStateQueued, snapshot.Conflicts[0].RequestState)
}
