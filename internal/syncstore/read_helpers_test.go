...
func TestReadDurableIntentCounts_ReadOnlyDB(t *testing.T) {
	...
	result, err := store.RequestConflictResolution(ctx, "conflict-pending", synctypes.ResolutionKeepLocal)
	require.NoError(t, err)
...
func TestHasScopeBlockAtPath(t *testing.T) {
	...
	hasAuthScope, err := HasScopeBlockAtPath(t.Context(), dbPath, synctypes.SKAuthAccount(), newTestLogger(t))
	require.NoError(t, err)
	assert.True(t, hasAuthScope)

	hasServiceScope, err := HasScopeBlockAtPath(t.Context(), dbPath, synctypes.SKService(), newTestLogger(t))
	require.NoError(t, err)
