...
func TestUpsertShortcut_PreservesDiscoveredAt(t *testing.T) {
	...
	sc := synctypes.Shortcut{
		...
		LocalPath: "path1", Observation: synctypes.ObservationDelta, DiscoveredAt: 1000,
	}
	require.NoError(t, mgr.UpsertShortcut(ctx, &sc))
...
func TestListShortcuts_Multiple(t *testing.T) {
	...
	for _, sc := range []synctypes.Shortcut{
		{ItemID: "sc-1", RemoteDrive: "d1", RemoteItem: "i1", LocalPath: "path1", Observation: synctypes.ObservationDelta, DiscoveredAt: 1000},
		{ItemID: "sc-2", RemoteDrive: "d2", RemoteItem: "i2", LocalPath: "path2", Observation: synctypes.ObservationEnumerate, DiscoveredAt: 2000},
	} {
		require.NoError(t, mgr.UpsertShortcut(ctx, &sc))
	}
