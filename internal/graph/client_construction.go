...
func NewClient(
	...
	return &Client{
		...
		policies: retryPolicies{
			driveDiscovery:       retry.DriveDiscoveryPolicy(),
			rootChildren:         retry.RootChildrenPolicy(),
			downloadMetadata:     retry.DownloadMetadataPolicy(),
			createFolderReadback: retry.PathVisibilityPolicy(),
			simpleUploadMtime:    retry.SimpleUploadMtimePatchPolicy(),
			uploadSessionCreate:  retry.UploadSessionCreatePolicy(),
			copyDestination:      retry.CopyDestinationPolicy(),
			simpleUploadCreate:   retry.SimpleUploadCreatePolicy(),
		},
		validators: urlValidators{
			uploadURL:   validateUploadURL,
			copyMonitor: validateCopyMonitorURL,
			socketIO:    validateSocketIONotificationURL,
		},
	}, nil
}
