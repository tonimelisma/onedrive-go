...
- R-5.6.5: After simple upload (files <= 4 MiB), the system shall perform a PATCH to `UpdateFileSystemInfo` to preserve the local modification timestamp. Simple upload (PUT to `/content`) sends raw binary with no way to include metadata in the same request, so the server stamps the file with the server receipt time. The post-upload PATCH restores the correct local mtime. Because Graph can transiently return `404 itemNotFound` on that immediate follow-on item-ID PATCH even after the upload itself succeeded, the graph boundary shall own a bounded exact retry for that narrow case before surfacing failure. The current production budget is 8 total attempts with a 250ms base, 2x multiplier, no jitter, and 16s max. This only affects simple uploads — resumable sessions use R-5.6.4 for atomic timestamps. [verified]
...
- R-5.6.10: When a non-zero create-by-parent simple upload returns `404 itemNotFound` and the fallback `createUploadSession` path also exhausts the documented fresh-parent `404 itemNotFound` retry budget, the graph boundary shall retry the original simple upload against the same parent with a second bounded create-convergence policy before surfacing final not-found. The session-route budget remains the narrow permission oracle for shared-folder `403` disambiguation, while the longer final simple-upload budget hardens ordinary recently-created-parent creates against longer convergence windows. [verified]
