...
- R-5.6.5: After simple upload (files <= 4 MiB), the system shall perform a PATCH to `UpdateFileSystemInfo` to preserve the local modification timestamp. Simple upload (PUT to `/content`) sends raw binary with no way to include metadata in the same request, so the server stamps the file with the server receipt time. The post-upload PATCH restores the correct local mtime. Because Graph can transiently return `404 itemNotFound` on that immediate follow-on item-ID PATCH even after the upload itself succeeded, the graph boundary shall own a bounded exact retry for that narrow case before surfacing failure. The current production budget is 7 total attempts with a 250ms base, 2x multiplier, no jitter, and 8s max. This only affects simple uploads — resumable sessions use R-5.6.4 for atomic timestamps. [verified]
