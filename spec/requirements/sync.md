...
- R-2.4.3: The parent sync engine shall suppress embedded shared-folder link and shortcut placeholder items from normal content planning, own parent-scoped shortcut alias lifecycle for those facts, and surface parent-declared child runner actions to the namespace control plane. The control plane shall orchestrate automatic child runners from those actions, and no control-plane path shall rediscover parent-drive remote state through Graph. [verified]
...
- R-2.4.8: For selected standalone namespace mounts, the parent engine shall be the only parent-drive Graph remote-state observer and shortcut alias mutator. It shall surface OneDrive shortcut placeholders whose `remoteItem` points at a folder as parent-owned shortcut publication facts instead of content events, persist parent shortcut-root state in its sync store, and keep protected child-root paths suppressed from parent content planning. Complete remote shortcut batches are authoritative even when empty, while empty incremental shortcut batches are non-events. The control plane shall consume parent-declared runner actions, cache them only for runner orchestration, start `run` children, skip `skip_parent_blocked` children, and final-drain `final_drain` children. One-shot shall admit a parent's child set as soon as that parent publishes a fresh accepted live publication, without waiting for that parent pass or unrelated parents to return; final-drain and cleanup acknowledgements still wait for the same live parent to reach its safe acknowledgement point. Watch shall admit children from fresh live parent publications. Neither mode shall start automatic children from stale cached publication after parent startup failure or parent exit. Successful child final drain shall be acknowledged to the live parent engine; the parent must durably enter `removed_release_pending`, then release its protected alias root or promote a waiting same-path replacement before publishing `removed_child_cleanup_pending`; the parent cleanup request must carry the binding ID, child mount ID, local root, and cleanup reason, and multisync must reject incomplete cleanup requests instead of deriving scope. Child runner state DB paths and cleanup artifact paths shall be derived from the explicit orchestrator data directory, not ambient config defaults. Multisync then purges child-owned state artifacts and acknowledges cleanup so the parent can delete the cleanup-pending row. Failed or target-inaccessible final drain shall keep the child state DB and retry/block state for normal retry. A previously materialized child root shall store a filesystem identity; same-parent local shortcut alias rename is supported and mutates only the shortcut alias while preserving the child mount/state DB; local alias delete removes only the shortcut alias and never target content; cross-parent behavior is unsupported by design and shall not compare children across parent engines. Ambiguous/unsafe identity matches or mutation failures shall stay protected and surface concise status with typed issue/recovery classes, protected paths, auto-retry status, and required user action when any is needed. If persisting parent shortcut publication or parent shortcut-root lifecycle state fails, the parent engine shall not commit the remote observation cursor for those facts, startup shall continue durable standalone parents, and recovery shall retry from durable truth instead of blocking unrelated mounts. [verified]
