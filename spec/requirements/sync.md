...
- R-2.3.3: When the user runs `status`, the system shall present one per-drive read view for every displayed drive, including ordinary issue groups, delete-safety entries, unresolved conflicts, state-store health, and explicit next-step guidance for each actionable issue/delete/conflict/state-store condition. `--drive` shall only filter which drives are displayed; it shall not switch `status` into a different output shape. [verified]
- R-2.3.4: When the user runs `status --history`, the system shall include resolved conflict history for the currently displayed drives. [verified]
...
- R-2.3.10: When `--json` is passed to `status`, the JSON contract shall stay the same regardless of whether `--drive` is used. Each displayed drive's nested `sync_state` shall expose structured arrays for `issue_groups`, `delete_safety`, and `conflicts`, plus optional `conflict_history` when `--history` is selected, together with sampling metadata (`examples_limit`, `verbose`) and sampled-section totals. The JSON contract shall also expose per-entry `action_hint` where user action is possible, plus a deduplicated per-drive `next_actions` array. [verified]
...
- R-2.10.9: When periodic remote permission maintenance discovers a previously-denied remote capability is restored, the system shall clear the matching permission state. Shared-folder write denial shall clear `perm:remote-write` and release held actions under that boundary immediately; item-level `remote_read_denied` failures shall clear when the periodic read probe succeeds. [verified]
- R-2.10.10: Periodic capability-matched permission rechecks shall be the only mechanism that clears permission state. Scanner/watch batches and external DB mutations shall not clear permission failures or permission scopes. [verified]
...
- R-2.10.12: When a local file operation fails with `os.ErrPermission`, the system shall classify the failure by required local capability. If the relevant parent directory is inaccessible for that capability: record one recursive directory scope (`perm:local-read` or `perm:local-write`) and suppress matching operations under it. If the parent directory remains accessible: record an item-level `local_read_denied` or `local_write_denied` failure instead. [verified]
- R-2.10.13: The system shall recheck local permission state at the start of each sync pass and on the watch-mode maintenance tick using capability-matched probes, and auto-clear `perm:local-read`, `perm:local-write`, `local_read_denied`, and `local_write_denied` when the required access is restored. [verified]
...
- R-2.10.22: The per-drive `status` view shall identify shortcut-scoped 507 by local path name (e.g., "Shared folder 'Team Docs'"), not opaque drive IDs. [verified]
...
- R-2.10.34: The active `scope_key` format shall be: `auth:account`, `quota:own`, `quota:shortcut:$remoteDrive:$remoteItem`, `perm:remote-write:{localPath}`, `perm:local-read:{localPath}`, `perm:local-write:{localPath}`, `throttle:target:drive:$targetDriveID`, `throttle:target:shared:$remoteDrive:$remoteItem`, `service`, `disk:local`. Persisted scope keys use stable internal identifiers where required for correctness; human-readable naming for `status` output is derived at display time from shortcut metadata. Legacy `throttle:account`, `perm:remote:{localPath}`, and `perm:dir:{localPath}` keys may still be parsed during startup repair, but new runtime state shall not persist them. [verified]
