...
- R-2.10.33: The `sync_failures` table shall store a `scope_key` column for scope-level failures, enabling `issues` display grouping without re-deriving scope. [implemented]
...
- R-2.10.41: When a download, delete, or move action succeeds, the system shall clear any corresponding `sync_failures` entry for that path. [implemented]
