...
- R-2.4.3: The parent sync engine shall suppress embedded shared-folder link and shortcut placeholder items from normal content planning, own parent-scoped shortcut alias lifecycle for those facts, and surface parent-declared child process commands to the namespace control plane. The control plane shall orchestrate automatic child runners from those commands, and no control-plane path shall rediscover parent-drive remote state through Graph. [verified]
