...
- R-2.10.6: For `quota_exceeded` scope blocks, trial timing shall start at 5 minutes, double on each failed trial, with a maximum of 1 hour. [verified]
- R-2.10.7: For `rate_limited` scope blocks, trial timing shall start at the `Retry-After` duration from the server response. The scope block shall affect all action types for the account. [verified]
- R-2.10.8: For `service_outage` scope blocks, trial timing shall start at 60 seconds (or `Retry-After` if present), double on failure, with a maximum of 10 minutes. [verified]
...
- R-2.10.14: Trial timing shall be per scope type: `rate_limited` starts at Retry-After (max 10 min); `quota_exceeded` starts at 5 min (2x backoff, max 1 hour); `service_outage` starts at 60s or Retry-After (2x backoff, max 10 min). [verified]
