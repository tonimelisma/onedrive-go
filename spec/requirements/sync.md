...
- R-2.4.1: Per-drive `included_dirs`, `ignored_dirs`, `ignored_paths`, `ignore_dotfiles`, and `ignore_junk_files` shall define sync-only content visibility. The planner shall compare raw `baseline` against filtered current local and remote views, and a path hidden from both current views shall produce baseline cleanup rather than local or remote delete work. [verified]
- R-2.4.2: Local observation shall persist only visible/admissible local snapshot rows. Remote observation shall persist raw manageable remote rows and apply product visibility only at planner view and wake-optimization boundaries, so removing a filter can reveal already-observed remote truth. [verified]
- R-2.4.3: The parent sync engine shall suppress embedded shared-folder link and shortcut placeholder items from normal content planning, own parent-scoped shortcut alias lifecycle for those facts, and surface parent-declared child work commands to the namespace control plane. The control plane shall orchestrate automatic child runners from those commands, and no control-plane path shall rediscover parent-drive remote state through Graph. [verified]
- R-2.4.4: Observation shall not use marker-file-driven scope changes. Marker-looking files such as `.nosync` are ordinary files unless explicit config filters match them. [verified]
- R-2.4.5: `included_dirs` shall narrow a drive to configured root-relative directory scopes, their descendants, and necessary ancestor containers. `ignored_dirs` and `ignored_paths` shall remove matching subtrees/paths, and ignore rules shall win over include rules. [verified]
- R-2.4.6: Symlinks shall be skipped by default. When `follow_symlinks=true`, symlink targets may be observed at the alias path with cycle protection; remote delete of the alias removes only the symlink, and descendant deletes under a symlinked directory are blocked rather than deleting target content. [verified]
...
- R-2.4.8: For selected standalone namespace mounts, the parent engine shall be the only parent-drive Graph remote-state observer and shortcut alias mutator. It shall surface OneDrive shortcut placeholders whose `remoteItem` points at a folder as parent-owned shortcut facts instead of content events, persist parent shortcut-root state in its sync store, and keep protected child-root paths suppressed from parent content planning. Complete topology batches are authoritative even when empty, while empty incremental topology batches are non-events. The control plane shall consume exact parent-declared child work snapshots, cache them only for the active one-shot/watch runtime, start one-shot child work only after the publishing parent reaches its safe point, admit watch children only from fresh live parent work snapshots, and run final-drain child work commands in bidirectional full-reconcile mode. Parent-blocked shortcuts shall appear only in sync-owned status rows, not in multisync runner commands. One-shot shall not wait for unrelated parents before starting a completed parent's child work, but shall not start a child from a merely cached or still-running parent publication. Neither mode shall start automatic children from stale cached snapshot after watch startup, parent startup failure, parent exit, or a later run. Successful child final drain shall be acknowledged to the live parent engine through an opaque acknowledgement reference; the parent must durably enter `removed_release_pending`, then release its protected alias root or promote a waiting same-path replacement before publishing `removed_child_cleanup_pending`; the parent cleanup command must carry an opaque acknowledgement reference, child mount ID, local root, and cleanup reason, and multisync must reject incomplete cleanup commands instead of deriving scope. Multisync then purges child-owned state artifacts from its explicit `DataDir` and acknowledges cleanup so the parent can delete the cleanup-pending row; cleanup failures shall surface domain classes that distinguish remaining artifacts from parent-ack failures, and transient cleanup diagnostics shall clear after a successful retry or after the current parent snapshot has no cleanup work for that source. Failed or target-inaccessible final drain shall keep the child state DB and retry/block state for normal retry. A previously materialized child root shall store a filesystem identity; same-parent local shortcut alias rename is supported and mutates only the shortcut alias while preserving the child mount/state DB; local alias delete removes only the shortcut alias and never target content; cross-parent behavior is unsupported by design and shall not compare children across parent engines. Ambiguous/unsafe identity matches or mutation failures shall stay protected and surface concise status with typed issue/recovery classes, protected paths, auto-retry status, and required user action when any is needed. If persisting parent child work snapshot or parent shortcut-root lifecycle state fails, the parent engine shall not commit the remote observation cursor for those facts, startup shall continue durable standalone parents, and recovery shall retry from durable truth instead of blocking unrelated mounts. [verified]
...
- R-2.4.10: Shortcut projection conflicts shall be durable parent-owned lifecycle facts. Duplicate automatic child projections for the same namespace/content root are marked by the parent engine as `duplicate_target` using a deterministic path/binding winner. Explicit standalone shared-folder mounts may project the same remote content root as automatic shortcut children when their configured local `sync_dir`s do not overlap; config validation owns duplicate/nested configured sync-dir rejection. Local child-root file, final-symlink, symlinked-ancestor, or traversal collisions are recorded in parent `shortcut_roots` instead of letting the child engine start on an unsafe path. [verified]
- R-2.4.11: Local watch prefiltering shall treat filesystem events with unknown kind as directory-capable as well as file-capable. Included directory roots and their necessary ancestors must pass the pre-stat filter so the later observation stage can refresh authoritative local truth for that path. [verified]
...
- R-2.8.6: When watch mode has a pending replan, the engine shall keep not-yet-started work engine-owned, retire queued old-runtime outbox actions instead of dispatching them, and retire dependents released by already-running old actions instead of appending them back to dispatch. A replan signal that is already ready shall be consumed before old-outbox dispatch is enabled for the current watch-loop step. Retired old-runtime work shall not be recorded as success or ordinary retry work; the replacement runtime shall be rebuilt from current truth and durable retry/block state once already-running work drains. Whether a ready replan is consumed immediately while idle or after pending old work drains, a recoverable local-observation failure before replacement installation shall reschedule the same dirty/full-refresh intent instead of restoring or completing retired work. [verified]
- R-2.8.7: When a planned action reaches the runtime but current execution preconditions prove that exact action is obsolete, the engine shall classify the outcome as `superseded`. Superseded actions shall not count as success, shall not persist or keep `retry_work`, shall not activate or extend block scopes, shall not release dependents as runnable old-plan work, and shall schedule watch-mode replan from current truth when watch mode is active. [verified]
- R-2.8.8: Watch-mode local observation shall update durable `local_state` from engine-applied scoped observation batches before the next replan. File create/modify observations shall upsert exact rows, file deletes shall delete exact rows, directory create observations shall upsert observed subtree rows, directory delete/rename-away observations shall delete the path prefix, and safety scans shall replace the full local snapshot. Dropped local observation batches, dropped hash requests, watcher errors, and failed safety scans shall mark local truth suspect in `observation_state` and schedule a normal dirty replan/full local refresh; the local observer shall not write SQLite directly or create a separate event journal. [verified]
- R-2.8.9: Before dispatching ready work and again when a worker receives already-submitted work, the system shall validate planned action assumptions against latest committed truth, including both source and destination endpoints for move actions. Local-state mismatches shall supersede only when `observation_state.local_truth_complete` makes local absence/presence meaningful; suspect local truth shall not reject from `local_state`. Remote-state mismatches shall supersede when committed remote truth proves the planned item/path/hash/eTag assumptions false. Superseded stale-start work shall not dispatch, shall not execute side effects, shall not persist ordinary retry work, shall not release old-plan dependents, and shall schedule watch-mode replan when watch mode is active. [verified]
