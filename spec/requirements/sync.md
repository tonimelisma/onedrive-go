...
- R-2.3.3: When the user runs `status`, the system shall present one per-drive read view for every displayed drive, including all durable sync conditions and sync-state snapshots. `--drive` shall only filter which drives are displayed; it shall not switch `status` into a different output shape. [designed]
...
- R-2.3.10: When `--json` is passed to `status`, the JSON contract shall stay the same regardless of whether `--drive` is used. Each displayed drive's nested `sync_state` shall expose structured `conditions` together with sampling metadata (`examples_limit`, `verbose`) and sampled-section totals. [designed]
...
- R-2.10.4: When displaying sync status, the system shall show grouped condition families per drive with scope context (file, directory, drive, account, service) together with retry and blocker information. Text output shall render those groups human-readably, and `--json` shall preserve them structurally with summary key, count, scope kind, and optional humanized scope label. [designed]
- R-2.10.5: When a block scope is active, the system shall test for recovery by periodically dispatching one real blocked action as a trial. On success: clear the block scope and unblock matching `retry_work`. On failure that proves the same scope still persists: keep the blocked work and extend the trial interval. On inconclusive trial outcomes: preserve the current scope interval and do not treat missing usable candidates as proof of recovery. [designed]
...
- R-2.10.9: When `recheckPermissions()` discovers a previously-denied folder is now writable, the system shall clear the block scope and release all held actions under that folder immediately. [verified]
...
- R-2.10.11: When a block scope clears (via trial success or recheck), the system shall release all held actions immediately with `NotBefore` set to now. Individual action `failure_count` shall be preserved. [verified]
...
- R-2.10.13: The system shall recheck `local_permission_denied` directory-level conditions at the start of each sync pass, and auto-clear when the directory becomes accessible. [verified]
- R-2.10.14: Trial timing uses default constants of 5 seconds initial, 2× backoff, 5-minute cap for locally timed quota/service scopes. Server-provided Retry-After values are honored exactly with no cap. Preserve outcomes re-arm the current interval without incrementing `trial_count`. Blocker timing is owned by `block_scopes`, and scope-specific overrides may define different local timing curves such as `disk:local` in R-2.10.43. [designed]
...
- R-2.10.25: When a configured shared-root drive's root itself is read-only, the system shall record a block scope at that configured root level without walking above it. [verified]
...
- R-2.10.32: The `status` command shall show per-block scope status as separate entries per drive, preserving separate condition groups even when the summary key is the same. [designed]
- R-2.10.33: Shared blocker timing and scope identity shall live in `block_scopes`, while exact blocked work shall live in `retry_work`, so `status` can group scoped conditions without depending on a mixed failure table. [designed]
...
- R-2.10.41: When a download, delete, or move action succeeds, the system shall clear any corresponding retry-work or durable current-truth status rows that no longer apply to that path. [designed]
...
- R-2.10.43: When available disk space falls below `min_free_space`, the system shall set a `disk:local` block scope suppressing all downloads. Trial timing shall start at 5 minutes, double on failure, with a maximum of 1 hour. [verified]
