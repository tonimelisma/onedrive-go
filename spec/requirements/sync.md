...
- R-2.3.3: When the user runs `status`, the system shall present one per-drive read view for every displayed drive, including ordinary issue groups and durable sync-state snapshots. `--drive` shall only filter which drives are displayed; it shall not switch `status` into a different output shape. [verified]
...
- R-2.3.7: When `status` encounters more than 10 failures of the same issue type for a displayed drive, the system shall group them under a single heading with count and show the first 5 paths. When `--verbose` is passed, the system shall show all paths. Default sampling shall apply equally to text and JSON output. [verified]
- R-2.3.8: When displaying scope-level issues where drives have independent scopes, the system shall preserve those groups distinctly within each drive's status output. Separately configured shared-root drives appear as separate drives rather than embedded nested scopes. [verified]
...
- R-2.3.10: When `--json` is passed to `status`, the JSON contract shall stay the same regardless of whether `--drive` is used. Each displayed drive's nested `sync_state` shall expose structured `issue_groups` together with sampling metadata (`examples_limit`, `verbose`) and sampled-section totals. [verified]
...
- R-2.5.1: When the process is killed mid-sync, the next run shall resume cleanly by reloading durable truth (`baseline`, `remote_state`, `sync_failures`, `scope_blocks`, and catalog-owned account auth state), reobserving current state, and replanning from that truth. [verified]
...
- R-2.5.5: Sync startup shall own the only supported sync-state recovery path. When an existing per-drive state DB cannot be opened under the current schema, or contains unsupported legacy persisted state, startup shall recreate it once from a fresh canonical schema and continue without a manual repair command. [verified]
- R-2.5.6: The sync state DB shall bootstrap directly into the current canonical schema, reject non-canonical existing state stores loudly at the store boundary, and let sync startup recreate unusable stores automatically while read-only inspectors remain non-mutating. [verified]
...
- R-2.10.4: When displaying sync status, the system shall show grouped visible issue families per drive with failure scope context (file, directory, drive, account, service) alongside retry information. Text output shall render those groups human-readably, and `--json` shall preserve them structurally with summary key, count, scope kind, and optional humanized scope label. [verified]
...
- R-2.10.16: Every `WorkerResult` shall carry target drive identity (`TargetDriveID`) so scope classification and target-scoped retries can route results to the correct configured drive boundary. [verified]
...
- R-2.10.20: When 507 occurs on a separately configured shared-root drive, it shall block uploads only for that configured drive. Other configured drives shall continue independently. [verified]
...
- R-2.10.23: When 403 occurs on a separately configured shared-root drive, permission boundary walking shall use that drive's configured remote root and drive identity, not another drive's root. [verified]
...
- R-2.10.25: When a configured shared-root drive's root itself is read-only, the system shall record a scope block at that configured root level without walking above it. [verified]
...
- R-2.10.27: When a 429 target scope clears, only the held actions for that exact throttled target shall be released. Unrelated drives shall remain unaffected. [verified]
- R-2.10.28: When 5xx scope blocks are active, they shall affect all drives, since the Graph API is shared infrastructure. [verified]
...
- R-2.10.30: During `service` scope blocks, the system shall suppress remote observation polling globally. During drive-target throttle blocks, suppression shall remain limited to the affected drive/target boundary. [verified]
- R-2.10.31: During `quota:own` scope blocks, observation of the affected drive shall continue read-only. Other configured drives shall be unaffected. [verified]
- R-2.10.32: The `status` command shall show per-scope block status as separate entries per drive, preserving separate visible issue groups even when the summary key is the same. [verified]
...
- R-2.10.34: The `scope_key` format shall be: `quota:own`, `perm:remote:{localPath}`, `perm:dir:{localPath}`, `throttle:target:drive:$targetDriveID`, `service`, `disk:local`. Persisted scope keys use stable internal identifiers where required for correctness. Unsupported legacy scope-key shapes are not migrated in place; startup recreates that DB instead. [verified]
