...
- R-2.10.5: When a block scope is active, the system shall test for recovery by periodically dispatching one real blocked action as a trial. On success: clear the block scope and unblock matching `retry_work`. On failure that proves the same scope still persists: keep the blocked work and extend the trial interval. On inconclusive trial outcomes: re-arm the current scope interval without incrementing `trial_count`, but only while blocked `retry_work` still exists for that scope. If refresh or cleanup leaves the scope with no blocked work, discard the scope immediately. [designed]
