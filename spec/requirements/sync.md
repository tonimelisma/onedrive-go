...
- R-2.3.3: When the user runs `status`, the system shall present one per-drive read view for every displayed drive, including ordinary issue groups and state-store health. `--drive` shall only filter which drives are displayed; it shall not switch `status` into a different output shape. [verified]
...
- R-2.10.34: The `scope_key` format shall be: `auth:account`, `quota:own`, `perm:remote:{localPath}`, `perm:dir:{localPath}`, `throttle:target:drive:$targetDriveID`, `service`, `disk:local`. Persisted scope keys use stable internal identifiers where required for correctness. Legacy `throttle:account` and `auth:account` keys may still be parsed during startup repair, but new runtime state shall not persist them. [verified]
