...
- R-2.4.8: For selected standalone namespace mounts, the control plane shall discover OneDrive shortcut placeholders whose `remoteItem` points at a folder, persist a managed child mount in `mounts.json`, apply any pending local projection move before creating the new child root, create or validate the child local root before starting that child engine, and keep the parent engine from owning the reserved child subtree. A never-materialized child root may be created. A previously materialized child root shall store a filesystem identity; if that root is renamed to exactly one sibling path, the control plane shall PATCH only the shortcut placeholder alias and preserve the child mount/state DB; if that root is locally deleted, the control plane shall DELETE only the shortcut placeholder and purge only the child state. Ambiguous/unsafe identity matches or mutation failures shall become durable child conflict/unavailable states. If persisting a child-root reconciliation result fails after in-memory classification, startup shall continue durable standalone parents and unchanged durable children, skip dirty child records whose state was not saved, and retry persistence on a later reconciliation instead of blocking unrelated mounts. [verified]
