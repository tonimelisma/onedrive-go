...
- R-2.3.3: When the user runs `status` with exactly one selected drive, the system shall present one detailed per-drive read view that includes ordinary issue groups, delete-safety entries, unresolved conflicts, state-store health, and explicit next-step guidance for each actionable delete/conflict/state-store condition. [verified]
...
- R-2.3.10: When `--json` is passed to single-drive `status`, the detailed read model shall expose structured arrays for `issue_groups`, `delete_safety`, and `conflicts`, plus optional `conflict_history` when `--history` is selected. The JSON contract shall also expose per-entry `action_hint` where user action is possible, plus a deduplicated top-level `next_actions` array for the selected drive. [verified]
