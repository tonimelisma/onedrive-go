...
- R-3.6.5: When shared-drive discovery needs the deprecated `sharedWithMe` fallback, the system shall request external shares with `allowexternal=true`. If Microsoft Graph still omits an external or cross-organization shared folder after that discovery attempt, the CLI shall explain the platform limitation clearly and point the user at discovery commands that show what the API actually exposed. [verified]
