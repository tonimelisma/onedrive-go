...
## R-3.2 Drive Types [implemented]

The system shall support all four drive types:

- R-3.2.1: OneDrive Personal (consumer Microsoft accounts). [implemented]
- R-3.2.2: OneDrive Business (Microsoft 365 / Azure AD work accounts). [implemented]
- R-3.2.3: SharePoint Document Libraries (via `drive add`). [implemented]
- R-3.2.4: Shared Folders (folders shared by other users, synced as separate drives). [implemented]

## R-3.3 Drive Management Commands [implemented]

- R-3.3.1: When the user runs `drive list`, the system shall show all configured drives with status. [implemented]
- R-3.3.2: When the user runs `drive add`, the system shall add a SharePoint library or shared folder. [implemented]
- R-3.3.3: When the user runs `drive remove`, the system shall remove the drive config section. [implemented]
- R-3.3.4: When the user runs `drive search`, the system shall search SharePoint sites by name. [implemented]
...
## R-3.5 Drive Selection [implemented]

- R-3.5.1: When `--drive` is passed, the system shall match by canonical ID, display name, or substring. [implemented]
- R-3.5.2: When `--account` is passed on auth commands, the system shall select by email. [implemented]
- R-3.5.3: `--drive` shall be repeatable for sync/status to select multiple drives. [implemented]
