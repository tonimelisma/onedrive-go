...
- R-6.7.14: ~~When the Graph API returns HTTP 400 with a Graph error code chain containing `invalidRequest` and known outage message patterns (e.g., "ObjectHandle is Invalid"), the system shall classify it as transient (service outage) and route through scope-based retry, not generic skip.~~ [cancelled — one historical incident was documented, but the repo no longer has recoverable payload evidence for the exact shape. The runtime does not special-case this 400 until it reappears with captured data.]
