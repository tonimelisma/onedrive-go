...
- R-6.8.5: The system shall parse `RateLimit-Remaining` headers and proactively slow when less than 20% remains. [implemented]
- R-6.8.6: The system shall honor `Retry-After` from both 429 and 503 responses, populating `GraphError.RetryAfter`. [implemented]
- R-6.8.7: During sync, workers shall never block on retry backoff. Actions shall be returned to the tracker with `NotBefore` and incremented attempt count. Workers block only for HTTP round-trip time. [verified]
...
- R-6.8.10: Each sync action shall have an in-memory retry budget (default 5 attempts in one-shot mode, unlimited in watch mode). When exhausted (one-shot), the action is completed as failed and recorded in `sync_failures` as a diagnostic record. In watch mode, the tracker retries forever with increasing backoff. `sync_failures` is diagnostic-only — it never drives retry scheduling. [verified]
- R-6.8.11: The system shall use unified non-compounding backoff: `min(1s * 2^attempt, 5min)`. Single mechanism — no reconciler handoff. [verified]
...
- R-6.8.15: The engine shall classify the following HTTP status codes as transient: 5xx (issue_type: server_error), 408 (request_timeout), 412 (transient_conflict), 404 (transient_not_found), 423 (resource_locked). Transient errors are retried via tracker re-queue (unified backoff: `min(1s * 2^attempt, 5min)`). When retries are exhausted (one-shot mode, 5 attempts), the error is recorded in sync_failures with the specific issue_type for diagnostic grouping in the issues command. HTTP 423 (SharePoint co-authoring lock) was previously classified as skip — the non-blocking retry architecture handles multi-hour locks naturally via tracker re-queue without dedicated timeout logic. [verified]
