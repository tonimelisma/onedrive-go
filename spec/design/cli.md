...
- **Derived shared-folder issues**: `perm:remote-write` is displayed from held blocked-write rows, not from a standalone boundary issue. The CLI shows one visible issue per denied boundary only while blocked write intent still exists.
