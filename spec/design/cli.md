...
- Browser auth URLs are validated against loopback or Microsoft auth hosts before launching the platform browser command. The remaining inline `gosec` suppression on the `exec.CommandContext` call is intentional: the command comes from a fixed allowlist, but the linter cannot prove that through the helper boundary.
