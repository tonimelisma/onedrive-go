...
| Parent shortcut-root topology state is stored in the parent sync store, merged into startup reservations, and applied before topology facts are acknowledged to multisync. Empty complete topology batches are persisted and retire old roots; child final-drain acknowledgement removes retired roots or promotes waiting replacements. | `TestSyncStore_ApplyShortcutTopologyPersistsParentShortcutRoots`, `TestSyncStore_EmptyCompleteShortcutTopologyMarksRemovedFinalDrain`, `TestSyncStore_SamePathReplacementWaitsBehindRetiringRoot`, `TestSyncStore_AcknowledgeShortcutChildFinalDrainRemovesRetiredRoot`, `TestSyncStore_AcknowledgeShortcutChildFinalDrainPromotesWaitingReplacement`, `TestNewMountEngine_MergesPersistedShortcutRootReservations`, `TestApplyShortcutTopologyBatch_PersistsParentStateBeforeHandler` |
