...
- **Trial actions**: Scope block recovery via real action probes. `TrackedAction` has `IsTrial` and `TrialScopeKey` fields. `runTrialDispatch` picks the oldest scope-blocked failure from `sync_failures` via `PickTrialCandidate` and synthesizes a re-observation event into the buffer. `scopeGate.NextDueTrial(now)` returns the first due scope. `scopeGate.ExtendTrialInterval(key, nextAt, interval)` extends on failure. Success → `onScopeClear` (sets `next_retry_at = NOW` for all scope-blocked items). Failure → interval doubled with 2× backoff, capped per scope type. Per-scope-type timing: rate_limited uses `Retry-After`; quota: 5m→1h; service_outage: 60s→10m.
