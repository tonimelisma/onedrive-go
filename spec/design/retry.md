...
| `Reconcile` | Single retry curve for all sync failures (`sync_failures` + drain-loop retrier) | 0 (infinite) | 1s | 1h |
...
- **Single retry mechanism**: The `sync_failures` table + drain-loop retrier (`runRetrierSweep` in engine.go) is the sole retry mechanism for sync actions. The DepGraph is purely a dependency graph — no retry logic (no ReQueue, no delayed queue, no NotBefore, no Attempt, no MaxAttempts). Failed items are recorded in `sync_failures` with `next_retry_at` computed via `retry.Reconcile.Delay`, and the drain-loop retrier re-injects due items via buffer → planner → DepGraph.
...
- **No transport-level retry for sync**: Sync callers use raw `http.DefaultTransport`. Each sync dispatch = one HTTP request. Workers never block on retry backoff. Failed actions are recorded in `sync_failures` with `next_retry_at` computed via `retry.Reconcile.Delay`, and the drain-loop retrier (`runRetrierSweep`) re-injects due items.
- **Single retry mechanism**: `sync_failures` (SQLite) + drain-loop retrier (`runRetrierSweep`). The DepGraph is purely a dependency graph — no retry logic. The engine always calls `depGraph.Complete()` on every result (never `ReQueue`). Transient failures are recorded with `retry.Reconcile.Delay` (1s base, 2× multiplier, ±25% jitter, 1h max, infinite attempts). Actionable/fatal failures are recorded with nil `delayFn` (no `next_retry_at`).
- **Drain-loop retrier** (`runRetrierSweep`): Runs inline in the drain loop select, batch-limited to `retryBatchSize`. Sweeps `sync_failures` for items whose `next_retry_at` has expired. Re-injects as synthetic `ChangeEvent`s via buffer → planner → DepGraph. Skips items already in-flight via `depGraph.HasInFlight`. Arms a timer for the earliest future retry.
- **Trial actions**: Scope block recovery via real action probes. `TrackedAction` has `IsTrial` and `TrialScopeKey` fields. `runTrialDispatch` picks the oldest scope-blocked failure from `sync_failures` via `PickTrialCandidate` and synthesizes a re-observation event into the buffer. `scopeGate.AllDueTrials(now)` returns a snapshot of all due scope keys (structurally bounded iteration). `reobserve` returns `(*ChangeEvent, time.Duration)` — the `RetryAfter` is forwarded to `extendTrialInterval` for server-driven backoff (R-2.10.7). On successful dispatch, the trial interval is NOT extended (awaits worker result). Success → `onScopeClear` (sets `next_retry_at = NOW` for all scope-blocked items). Failure → interval doubled with 2× backoff, capped per scope type. Per-scope-type timing: rate_limited uses `Retry-After`; quota: 5m→1h; service_outage: 60s→10m.
