...
Named policies are exposed as zero-arg accessors returning fresh `Policy` values. This prevents package-level mutable retry state while keeping call sites explicit.

The `ReconcilePolicy()` policy defines the single retry curve for all transient sync failures. Parameters: 1s base, 2× multiplier, ±25% jitter, 1h max, infinite attempts. Curve: 1s, 2s, 4s, 8s, ..., 3600s cap. The engine passes `retry.ReconcilePolicy().Delay` as the `delayFn` argument to `SyncStore.RecordFailure()` for transient failures. The store computes `next_retry_at` from the failure count without importing the retry package — it receives `delayFn` as a `func(int) time.Duration`.
...
Architecture:
- **CLI callers**: `RetryTransport{Inner: http.DefaultTransport, Policy: TransportPolicy()}` → 5 retries with exponential backoff
...
- **Single retry mechanism**: The `sync_failures` table + engine retry sweep (`runRetrierSweep` in `engine.go`) is the sole retry mechanism for sync actions. The DepGraph is purely a dependency graph — no retry logic (no ReQueue, no delayed queue, no NotBefore, no Attempt, no MaxAttempts). Failed items are recorded in `sync_failures` with `next_retry_at` computed via `retry.ReconcilePolicy().Delay`, and the retry sweep re-injects due items via buffer → planner → DepGraph.
- **Transient failures retry forever**: The `ReconcilePolicy()` policy has `MaxAttempts = 0` (infinite) with exponential backoff capped at 1 hour. Crash recovery is handled by `ResetInProgressStates` on startup.
