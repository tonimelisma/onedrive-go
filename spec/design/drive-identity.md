...
- Source of Truth: Canonical IDs derived from authenticated account data plus catalog-backed drive identity; display names are config-owned user-facing labels layered on top of those identities.
...
from the offline account view before any live search is attempted. The
owning account-view snapshot load is also where best-effort `/me` reconciliation runs for
...
degraded. If live search succeeds but Graph still omits an external or
...
- `driveid` stays pure value logic. Runtime drive-ID resolution is config-owned:
  ...
  while personal/business/SharePoint drives use catalog-backed drive identity
