...
- **Watch mode**: `hasCaseCollisionCached()` in `handleCreate` and `hashAndEmit` (`observer_local_handlers.go`) performs early rejection using a per-directory name cache. The cache is built lazily on first access to a directory, invalidated on Create/Delete events that change directory contents, and cleared entirely on each safety scan. `hasCaseCollision()` uses `strings.ToLower` consistently with `detectCaseCollisions` for case folding.
- **Collision peer tracking**: A `collisionPeers` map tracks pairwise collision relationships (each collider points to its peer). When a collider is deleted (`handleDelete`), the surviving peer is re-emitted as a `ChangeCreate` event so the planner can upload the now-unambiguous file.
