...
**Single-path local reconstruction** — `ObserveSinglePath()` rebuilds one local path from current truth for engine-owned retry/trial work. It takes a rooted `synctree.Root` instead of a raw sync-root string and applies the same per-path rules as normal observation: `ShouldObserve`, oversized-file rejection, baseline-hash reuse, and “emit with empty hash” behavior when hashing fails. Internal exclusions resolve the retry/trial candidate silently; actionable validation failures become `SkippedItem`s that the engine records into `sync_failures`.
