...
- Watch-mode websocket frames are wake signals only. `RemoteObserver` still owns the current delta token, zero-event token guard, wake-driven polling/backoff, and blocking emission of the already-committed batch. Websocket notifications never carry authoritative remote state and never advance durable sync state by themselves.
- Watch mode no longer mutates `RemoteObserver` with engine callbacks. The engine-owned root-delta batch handler now applies remote scope, commits `remote_state` rows plus the delta token atomically, runs shortcut follow-up that depends on committed primary-drive truth, and only then returns the final planner-visible batch for emission.
...
- Raw fsnotify behavior for those marker and path-boundary cases is captured
  ...
  `dir_move_into_scope`, and `dir_move_out_of_scope`.
