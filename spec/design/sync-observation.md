...
- **Stage 1.5 (when kind is known):** Root directory-name `skip_dirs` and file-glob `skip_files` checks run as soon as the observer knows whether the candidate is a file or directory. Full scans and watch setup know that from `DirEntry`; watch create/write and retry reconstruction apply the same rules immediately after `Stat`.
...
- Filtering is asymmetric by design: the local observer filters by always-excluded patterns, user-configured `skip_dotfiles` / `skip_dirs` / `skip_files`, and OneDrive naming rules (upload validation), while the remote observer trusts the server. If OneDrive sends an item in a delta response, it exists remotely — filtering it would cause silent data loss. Remote-only temp files (e.g., uploaded via web UI) are intentionally allowed through to the planner.
