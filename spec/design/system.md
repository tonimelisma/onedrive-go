...
- The documentation model is machine-checked, not conventional. `devtool verify` fails when a production Go file has no governing design doc, when two docs claim one file at equal pattern specificity, when a `GOVERNS:` pattern matches nothing, or when a design doc cites a test that does not exist. The last rule matters most: a requirement marked verified on the strength of a renamed or deleted test is a false claim, and only tooling catches that reliably.
