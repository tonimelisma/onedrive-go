...
- Fast E2E is mandatory in the default local `default` profile. The harness loads root `.env` plus durable `.testdata/fixtures.env` defaults itself; verification does not silently skip fast E2E based on exported shell variables.
