...
- `go run ./cmd/devtool watch-capture --scenario <name> [--json]` captures raw
  per-OS fsnotify event sequences for scoped-watch scenarios into a stable
  JSON shape. The command is diagnostic tooling only; product CLI stays
