...
- Shared-root remote read helpers must re-establish that same `ls /` boundary before and after known route-lag failures. Owner-drive path visibility or an earlier successful shared-root listing does not prove later exact-path `stat`, `get`, or other shared-root read pollers will avoid the documented `items/{rootID}/children` `404 itemNotFound` family.
