...
- Every `synctree` filesystem call — read, write, and destroy alike — resolves its name through an open `os.Root` descriptor. Mutating operations are part of the rooted `rootHandle` surface specifically because they are the ones that destroy data when containment is skipped: a lexically validated absolute path handed to package-level `os.Rename` follows a symlinked ancestor out of the sync root and silently replaces whatever sits at the destination. `synctree` therefore exposes no absolute-path mutation entry point, and its injectable ops take a rooted handle plus a rooted-relative name so a future call site cannot reintroduce the escape.
