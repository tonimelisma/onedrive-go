...
- The `e2e_full` package runs serially (`go test -parallel 1`) because many of its tests mutate shared live-drive state, and parallelism there creates cross-test delta noise rather than useful signal. The verifier gives that serial package a `60m` timeout budget; the older `30m` cap was no longer compatible once the suite intentionally stopped overlapping mutating tests.
