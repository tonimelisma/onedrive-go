...
| `engine.go`, `engine_config.go`, `engine_run_once*.go`, `engine_watch*.go`, `engine_runtime_prepare*.go`, `engine_current_observe.go` | Runtime orchestration: one-shot results, current-truth observe/load, prepare handoff, watch bootstrap, direct watch-loop ownership, and steady-state replan flow |
