...
| `engine.go`, `engine_config.go`, `engine_run_once*.go`, `engine_watch*.go`, `engine_runtime_prepare*.go`, `engine_current_observe.go`, `engine_current_projection.go`, `engine_current_dry_run.go` | Runtime orchestration: one-shot results, current-truth observation and commits, planner-input loading, dry-run scratch planning, prepare handoff, shared watch-loop ownership across bootstrap/running/drain, and steady-state replan flow |
...
| `engine_result_classify.go`, `engine_runtime_completion*.go`, `engine_runtime_graph.go`, `engine_runtime_held.go`, `engine_results.go`, `engine_retry_trial.go` | Runtime completion: result classification, dep-graph completion/shutdown ownership, held-work release, and retry-trial timer ownership |
| `engine_runtime_invariants.go`, `engine_runtime_scope_startup.go`, `engine_runtime_scopes.go`, `engine_runtime_admission.go`, `scope_lifecycle_policy.go` | Runtime scope lifecycle: startup cleanup, timed blocker activation, release/discard, admission, and shared lifecycle policy for retry/write scopes |
...
| `permissions.go`, `permission_handler.go`, `permission_capability.go`, `engine_runtime_permissions.go`, `engine_runtime_permission_bridge.go`, `permission_probe_*.go` | Capability-based permission probing, denied-path policy, engine-owned permission application, and write-scope activation/release decisions |
