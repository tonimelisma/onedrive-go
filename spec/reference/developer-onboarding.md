...
| `engine.go`, `engine_config.go`, `engine_run_once*.go`, `engine_watch*.go`, `engine_runtime_prepare*.go`, `engine_current_observe.go`, `engine_current_inputs.go` | Runtime orchestration: one-shot results, current-truth observation and commits, planner-input loading, prepare handoff, shared watch-loop ownership across bootstrap/running/drain, and steady-state replan flow |
