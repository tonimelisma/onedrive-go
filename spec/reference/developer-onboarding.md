...
| `engine.go`, `engine_config.go`, `engine_run_once*.go`, `engine_watch*.go`, `engine_runtime_prepare*.go`, `engine_current_observe.go` | Runtime orchestration: one-shot results, current-truth observe/load, prepare handoff, watch bootstrap, watch loop/drain ownership, and steady-state replan flow |
...
| `engine_result_classify.go`, `engine_runtime_completion*.go`, `engine_runtime_held.go`, `engine_results.go`, `engine_retry_trial.go` | Runtime completion: result classification, completion routing, held-work release, and retry-trial timer ownership |
...
| `engine_runtime_state.go`, `engine_runtime_types.go`, `engine_time.go`, `engine_log_fields.go` | Engine runtime state, time helpers, and structured logging |
