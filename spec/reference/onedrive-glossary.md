...
### Sync Failure
A persistent record of a file that failed to sync. Two categories: **transient** (retried automatically with exponential backoff) and **actionable** (requires user intervention or an explicit safety approval such as `issues approve-deletes`, depending on the issue type). Stored in the `sync_failures` table.
