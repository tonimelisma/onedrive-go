...
**Phases 1, 2, 3, 3.5 complete. Phase 4v2 Increments 0-5 complete. Next: 4v2.6-4v2.8.** Old batch-pipeline sync engine deleted. `sync` command returns "not yet implemented" stub. `internal/sync/` rebuilt with foundation types, SQLite baseline schema (goose migrations), BaselineManager, Remote Observer (delta → ChangeEvent[]), Local Observer (FS walk → ChangeEvent[]), Change Buffer (event grouping + move dual-keying), and Planner (EF1-EF14/ED1-ED8 decision matrix, move detection, big-delete safety). All development happens on `clean-slate` branch (`main` is read-only safety net). See [docs/roadmap.md](docs/roadmap.md).
