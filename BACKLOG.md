...
| B-045 | Interface segregation for Store | P3 | `internal/sync/` | Store interface has 30+ methods. Consumers like Reconciler only need `ListAllActiveItems` + `IsDeltaComplete`. Consider `ReconcilerStore`, `ScannerStore` etc. to reduce mock boilerplate. |
