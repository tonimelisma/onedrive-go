...
| B-317 | API response fuzz tests | P3 | graph | **Scope:** Add Go fuzz tests (`func FuzzX`) for `graph.Item` JSON deserialization and observer field extraction. **Targets:** (1) `FuzzItemUnmarshal` — feed arbitrary JSON to `json.Unmarshal(&graph.Item{})`, verify no panics, validate invariants (non-empty ID when no error). (2) `FuzzClassifyItem` — feed fuzzed `graph.Item` structs to `classifyItem()` in `observer_remote.go`, verify no panics, validate that nil `File`/`Folder`/`Package`/`RemoteItem` fields are handled gracefully. (3) `FuzzDeltaResponse` — fuzz entire delta response JSON, verify `parseDeltaResponse()` returns error or valid result (no partial state). **Prerequisites:** None. **Deliverables:** 3 fuzz functions, seed corpus from real API responses (sanitized), CI integration via `go test -fuzz` with time limit. **Effort:** ~2 hours. |
| B-318 | Fault injection test suite | P3 | sync, driveops | **Scope:** Systematic fault injection tests for critical failure paths that lack coverage. **Targets:** (1) *Disk full during baseline commit:* Inject `ENOSPC` via mock `sql.DB` wrapper, verify SyncStore returns error without corrupting WAL. (2) *Network partition during upload session:* Inject `context.DeadlineExceeded` mid-chunk in `TransferManager.UploadFile()`, verify `.partial` cleanup and session record preservation for resume. (3) *SIGTERM during active worker pool:* Send `SIGTERM` to a goroutine running `ExecuteActions()`, verify graceful shutdown (in-flight actions complete, no orphaned `.partial` files). (4) *SQLite busy timeout exhaustion:* Inject `database is locked` error during `CommitOutcome()`, verify retry and eventual success or clean error. **Prerequisites:** May need injectable wrappers for `sql.DB` and filesystem operations. **Deliverables:** 4+ test functions with deterministic fault injection. **Effort:** ~4 hours. |
| B-319 | Clock skew resilience audit | P3 | sync | **Scope:** Audit and document all time-sensitive code paths, verify resilience to clock jumps (NTP corrections, VM migrations, daylight saving). **Targets:** (1) *Mtime comparisons in planner:* `PathView.Local.Mtime` vs `PathView.Baseline.Mtime` — verify monotonic where possible. (2) *Conflict timestamps:* `detected_at` and `resolved_at` in conflicts table — verify ordering invariants hold under backward clock jump. (3) *Session expiry in SessionStore:* `ExpiresAt` field checked in `TransferManager.ResumeUploadSession()` — verify expired sessions are discarded, not resumed with stale tokens. (4) *Retry backoff:* `time.After` in `withRetry()` — verify uses monotonic clock (Go guarantees this). (5) *SyncedAt baseline field:* Used for staleness detection — verify backward clock doesn't cause false "stale" classifications. **Deliverables:** Audit doc in `docs/design/`, test cases for identified vulnerabilities, `time.Now()` injection points where missing. **Effort:** ~3 hours. |
...
| ~~B-327~~ | ~~`EnsureDriveInConfig` broken for shared drives~~ | ~~P3~~ | **DONE** — Architecture A: `DriveTokenPath(cid)` no longer requires `cfg`. Shared drive token resolution reads drive metadata files (`drives/` subdir) for `account_canonical_id`. `addSharedDrive` registers drive metadata via `SaveDriveMetadata` before calling `EnsureDriveInConfig`. No more nil-cfg crashes. |
