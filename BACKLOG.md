...
| B-089 | Baseline concurrent-safe incremental cache for per-action commits | P1 | `internal/sync/` | Phase 5.0: `CommitOutcome()` is called by concurrent workers. `Baseline` gains `sync.RWMutex` + locked accessor methods: `GetByPath()` (RLock), `GetByID()` (RLock), `Put()` (Lock), `Delete()` (Lock), `Len()` (RLock). **Cross-cutting change**: all direct `baseline.ByPath[x]` accesses migrated to locked accessors. Affected files: `planner.go` (`buildPathViews`, `bigDeleteTriggered`), `executor.go` (`resolveParentID`), `verify.go` (`VerifyBaseline`), `observer_local.go`, `observer_remote.go`, `engine.go` (`resolveTransfer`). Maps remain public for test setup convenience but production code uses only locked accessors. Replaces old `Commit()` cache invalidate+reload pattern. |
| B-090 | Eliminate `createdFolders` map — use incremental baseline instead | P1 | `internal/sync/` | Phase 5.0: `createdFolders` is per-Executor (not shared across workers). With B-089, `CommitOutcome()` updates baseline via `Put()` immediately, so `resolveParentID()` finds newly-created folders via `GetByPath()`. DAG edges guarantee folder create completes before child dispatch. Delete `createdFolders` field, first branch of `resolveParentID()`, and `e.createdFolders[action.Path] = item.ID` write in `createRemoteFolder()`. |
| B-091 | `resolveTransfer()` calls batch `Commit()` — must migrate to `CommitOutcome()` | P1 | `internal/sync/` | Phase 5.0: `engine.go:384` calls `e.baseline.Commit(ctx, []Outcome{outcome}, "", ...)` for conflict resolution. When batch `Commit()` is deleted, this breaks at compile time. Change to `e.baseline.CommitOutcome(ctx, outcome, "")`. `CommitOutcome()` must handle empty `ledgerID` gracefully (skip ledger status update — no `action_queue` row exists for manual conflict resolution). |
