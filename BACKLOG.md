...
| B-050 | Upload creates dual DB rows instead of updating | P3 | `internal/sync/` | Scanner creates items with empty `DriveID`/`ItemID`. After upload, `updateUploadState` upserts with the server-assigned `ItemID`, creating a second row (different `ON CONFLICT` key). Phase 5 should address: either scanner sets temp UUID, or executor deletes old row after upload. |
