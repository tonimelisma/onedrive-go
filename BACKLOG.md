...
| B-044 | Replace stringly-typed folder create side with enum | P3 | `internal/sync/` | Reconciler uses `Action.NewPath = "local"/"remote"` to indicate which side needs a folder create (D2/D3/D5). Should be a `FolderCreateSide` enum. Address when implementing executor (4.7). |
