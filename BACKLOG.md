...
| B-047 | Audit remaining Store methods for `sql.ErrNoRows` leakage | P3 | `internal/sync/` | `GetItem` and `GetItemByPath` were fixed (PRs #60, #61) to return `(nil, nil)` instead of wrapped `sql.ErrNoRows`. `GetUploadSession` intentionally returns error (missing session is a genuine error). Audit all other query methods (`GetConflict`, checkpoint helpers) for consistent not-found semantics. |
