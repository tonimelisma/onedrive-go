...
| B-051 | Reconciler F8/F9 paths unreachable through real engine | P2 | `internal/sync/` | `ListAllActiveItems` filters `is_deleted=0`, but `MarkDeleted` sets `is_deleted=1`. After delta processor tombstones an item, the reconciler never sees it. F8 (remote tombstone → local delete) and F9 (edit-delete conflict) only work in reconciler_test.go mocks. Need to either include tombstoned items in query or redesign the local-delete path. |
