...
| B-095 | DepTracker.byPath cleanup on action completion | P3 | `internal/sync/` | Completed/canceled actions remain in `byPath`. Safe for per-cycle trackers (5.0), but long-lived trackers in watch mode (5.1+) could cancel the wrong action if the same path appears in consecutive cycles. Clean up `byPath` entries in `Complete()` and `CancelByPath()`. Linked to Phase 5.1/5.2. |
