...
| B-068 | Executor must fill zero DriveID for new local items | P1 | `internal/sync/` | For EF13/ED5 (new local file/folder upload), Action.DriveID is zero (`driveid.ID{}`) because the item has no remote observation and no baseline. The executor must fill DriveID from its per-drive Engine context before making API calls. Use `action.DriveID.IsZero()` assertion. |
