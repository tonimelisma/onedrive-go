...
| ~~B-011~~ | ~~Linux CI for diskspace.go field types~~ | ~~P2~~ | ~~`internal/sync/`~~ | **CLOSED**: Platform-specific build tags (`safety_darwin.go`, `safety_linux.go`) with appropriate syscall packages. Linux uses `golang.org/x/sys/unix.Statfs` to handle arch-specific field types. Verified via CI. PR #47. |
...
| ~~B-044~~ | ~~Replace stringly-typed folder create side with enum~~ | ~~P3~~ | ~~`internal/sync/`~~ | **CLOSED**: `FolderCreateSide` enum (`FolderCreateLocal`/`FolderCreateRemote`) added to `types.go`, `CreateSide` field added to `Action` struct. Reconciler uses enum instead of string. PR #49. |
| ~~B-045~~ | ~~Interface segregation for Store~~ | ~~P3~~ | ~~`internal/sync/`~~ | **CLOSED**: Four narrow interfaces (`ReconcilerStore`, `SafetyStore`, `ScannerStore`, `DeltaStore`) added to `types.go`. Each consumer accepts only what it needs. ~250 lines of stub boilerplate removed. PR #55. |
| ~~B-043~~ | ~~Track directories as items in scanner~~ | ~~P3~~ | ~~`internal/sync/`~~ | **CLOSED**: Scanner now tracks directories as `ItemTypeFolder` entries with `LocalMtime` set. Includes folder orphan detection via `detectFolderOrphans`. PR #51. |
| B-046 | Rename `ConflictRecord.RemoteHash` field | P3 | `internal/sync/` | Field name is generic but always holds QuickXorHash. Renaming requires a DB migration. Defer to 4.8 (conflict handler) where the full ConflictRecord API will be designed. |
| ~~B-047~~ | ~~Audit remaining Store methods for `sql.ErrNoRows` leakage~~ | ~~P3~~ | ~~`internal/sync/`~~ | **CLOSED**: All `sql.ErrNoRows` checks now use `errors.Is`. `GetUploadSession` fixed to return `(nil, nil)` for not-found (PR #62). All single-row getters (`GetItem`, `GetItemByPath`, `GetUploadSession`, `GetDeltaToken`, `IsDeltaComplete`, `GetConfigSnapshot`) now follow consistent pattern. |
| B-048 | `ErrorRetryable` treated as `ErrorSkip` in `dispatchPhase` | P2 | `internal/sync/` | Throttled 429s silently abandoned. Phase 5 RunWatch concern — retry logic deferred. |
| B-049 | Graph mock `GetItem` defaults to `(nil, nil)` but real `graph.Client` returns `(nil, ErrNotFound)` for 404 | P3 | `internal/sync/` | Engine mocks return `(nil, nil)` for not-found items. Real `graph.Client` returns `(nil, ErrNotFound)`. Mock consistency should match real behavior. |
| ~~B-050~~ | ~~Upload creates dual DB rows instead of updating~~ | ~~P1~~ | ~~`internal/sync/`~~ | **CLOSED**: Fixed in PR #65. `populateDriveID()` injects DriveID into action (not item). Executor captures pre-mutation key, injects DriveID, upserts new row, deletes stale scanner row via `DeleteItemByKey()`. 10 new tests. |
| ~~B-051~~ | ~~Reconciler F8/F9 paths unreachable through real engine~~ | ~~P2~~ | ~~`internal/sync/`~~ | **CLOSED**: Fixed in PR #66. New `ListItemsForReconciliation` query includes tombstoned items with `synced_hash != ''`. Scanner now sets DriveID directly (eliminates `populateDriveID`). E2E test `TestRunOnce_LocalDelete_EndToEnd` verifies F8 pipeline. |
| B-052 | Re-enable E2E tests in CI | P2 | CI | E2E tests disabled in `integration.yml` (PR #76) because local E2E requires active login. Re-enable after E2E test prerequisites are fixed (token availability, test account login). |
