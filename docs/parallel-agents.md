...
**Run these commands from the clean-slate repo BEFORE starting any task:**

```bash
cd ~/Development/onedrive-go

# 1. Sync clean-slate with GitHub
git fetch origin && git merge --ff-only origin/clean-slate
...
git worktree add ../onedrive-go-<type>-<task> -b <type>/<task-name> clean-slate
...
> - Do NOT return to the clean-slate repo until cleanup after merge
>
...
If your worktree is based on an older clean-slate, rebase:

```bash
git fetch origin clean-slate
git rebase origin/clean-slate
...
git worktree add ../onedrive-go-feat-other -b feat/other-task clean-slate
```
...
| Task | Command |
|------|---------|
| Sync clean-slate | `git fetch origin && git merge --ff-only origin/clean-slate` |
| Create worktree | `git worktree add ../onedrive-go-<type>-<task> -b <type>/<task> clean-slate` |
...
| Rebase | `git fetch origin clean-slate && git rebase origin/clean-slate` |
