...
| **DP-8: Filter scoping** | Per-drive only. No global filter defaults. | Different drives have fundamentally different content. Global defaults create confusing inheritance. See [MULTIDRIVE.md §10](MULTIDRIVE.md#10-filter-scoping). |
| **DP-9: Duplicate-source detection** | Warn (not block) when same shared folder synced from multiple places. | No data corruption — just wasted bandwidth/disk. Warning gives visibility. See [MULTIDRIVE.md §4](MULTIDRIVE.md#duplicate-source-detection-dp-9). |
| **DP-10: Permission display** | `drive list` shows `(read-only)` / `(read-write)` for shared content. | Proactive visibility reduces confusion from 403 errors during sync. See [MULTIDRIVE.md §3](MULTIDRIVE.md#drive-list). |
| **DP-11: Graph API eventual consistency resilience** | Hybrid approach: production retry for `Drives()` 403, E2E polling for 404 eventual consistency. | 403 on `/me/drives` is a domain-specific transient failure (token propagation) that belongs in production code. 404 on path-based queries after writes is an API consistency gap — production code already handles 404 correctly (skip), but E2E tests that write-then-read need polling to avoid false failures. Polling helpers (`pollCLIContains`, `pollCLIWithConfigContains`) use exponential backoff (500ms→4s cap, 30s timeout). |
