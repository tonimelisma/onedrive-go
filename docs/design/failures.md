...
These failures originate from local filesystem changes detected by the watcher or scanner. The delta cursor is **not involved** — the planner regenerates upload actions every cycle as long as the local file differs from baseline.
...
| **Parent not in baseline** | Rare (ordering issue) | Yes, next cycle | Seconds |
...
Uploads do not need a recovery mechanism beyond what already exists. The planner compares local files against baseline every cycle. If an upload failed, the file still differs from baseline, and the planner regenerates the upload action. This provides infinite retry with no additional mechanism.

The only gaps are:
- **No backoff**: a permanently-failing upload (bad filename) is retried every cycle forever, wasting one worker's time per cycle
...
2. **Database token** — committed only when a cycle has zero failures (`engine.go:989`)

The intended safety: if a cycle fails, the database token stays stale. On daemon restart, the stale token causes re-observation of the failed items.

### The bug

`watchCycleCompletion` (`engine.go:1006`) commits `remoteObs.CurrentDeltaToken()` — the **latest** in-memory token, not a per-cycle token. This means:
...
The "don't commit on failure" protection is defeated by the next successful cycle, which commits the latest token (past everything, including previous failures). This happens in one poll interval (typically 5 minutes).
...
The in-memory failure tracker (`failure_tracker.go`) suppresses items after 3 failures. This accelerates the bug — suppressed items don't count as failures, so the cycle succeeds and commits the token. But the bug exists independently of the tracker: even without suppression, the in-memory token advances past failed items, and the next cycle (with different items) commits it.
...
The tracker is in-memory only — lost on daemon restart. It was introduced to prevent "delta token starvation" (a permanently-failing item blocking the token forever). In practice, it accelerates the delta token bug by ensuring cycles containing the suppressed item succeed.
...
Items lost permanently from the delta stream after one failed cycle followed by one successful cycle. Root cause: `watchCycleCompletion` commits the latest in-memory token rather than a per-cycle token. Affects downloads and local operations triggered by remote changes.

### 2. Silent permanent failures (user experience)

Permanently-failing items (bad filenames, permission errors, non-empty directories) retry silently every cycle forever. The user has no visibility into what can't sync. Affects all action types.

### 3. Wasted work on permanent failures (efficiency)

Items that will never succeed (bad filename, revoked permissions) consume a worker slot every cycle. With 4+ workers and a 5-minute poll interval, this is ~12 seconds of wasted work per item per cycle — negligible unless many items are permanently broken.
...
The **database token** (`delta_tokens` table) is committed by `watchCycleCompletion` (`engine.go:1007`) only when a cycle has zero failures. This token is read on daemon startup to determine where to resume. It is the crash-recovery token.

### Why the safety is illusory

The design intent: "if a cycle fails, don't commit the token, so on restart we re-observe the failed items." This works if and only if no subsequent cycle ever commits a later token. But `watchCycleCompletion` always reads `remoteObs.CurrentDeltaToken()` — the latest in-memory value, not the token that was current when the cycle's batch was created.
...
The failure tracker's actual effect: it suppresses D within cycles that DO contain it (before the in-memory token advances past it). In the scenario where D keeps failing and appears in the same cycle as other items that succeed, the tracker causes the cycle to succeed (by suppressing D), committing the token past D even in the SAME cycle. This is faster item loss than the base bug.
...
2. **The daemon restarts and the DB token is still stale** — this only works if no successful cycle has committed a later token, which (per the bug above) typically happens within 5 minutes. In practice, this recovery path almost never works.
