...
**Cause**: All parallel E2E tests share one OneDrive account. The delta endpoint (`GET /drives/{driveID}/root/delta`) returns changes for the **entire drive** — there is no way to scope delta to a subfolder reliably. Folder-scoped delta (`GET /drives/{driveID}/items/{folderId}/delta`) only works on **OneDrive Personal** — on OneDrive for Business and SharePoint, the delta action is only available on the `root` item of a drive. This is a **documented API limitation** per [Microsoft's official release notes](https://learn.microsoft.com/en-us/onedrive/developer/rest-api/getting-started/release-notes?view=odsp-graph-online), not a bug. Our production workaround for shared folder sync (Phase 6.4) uses dual observation: folder-scoped delta for personal source drives, recursive children enumeration for business/SharePoint source drives. Since our CI uses Personal accounts, folder-scoped delta could theoretically be used for test isolation, but cross-test contamination remains the simpler root cause to address.
