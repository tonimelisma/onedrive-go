...
1. Set up worktrees for all Wave N agents (from clean-slate)
...
5. Sync clean-slate: `git fetch origin && git merge --ff-only origin/clean-slate`
...
- PR merged to clean-slate
- **Integration tests passed/failed on clean-slate** (wait for `integration.yml` — catches regressions unit tests miss)
...
Top-up work is done in a worktree+PR if it touches code, or directly on clean-slate if doc-only. All top-up work MUST be completed before presenting the retrospective.
...
Run the DOD Cleanup Check from CLAUDE.md. Confirm only `main` and `clean-slate` exist
locally, only `origin/main` and `origin/clean-slate` remotely, no stashes, no open PRs, no orphaned dirs.
...
- [ ] All PRs merged, CI green on clean-slate (including integration.yml)
