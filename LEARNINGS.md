...
### Cache-through pattern for SyncStore.Load()
`SyncStore.Load()` returns cached `*Baseline` if `m.baseline != nil`. `Commit()` invalidates (`m.baseline = nil`) before calling `Load()` to refresh. Safe because the `SyncStore` manages all DB access via typed sub-interfaces with WAL mode + busy timeout for concurrency safety. For N conflict resolutions, this turns 2N DB loads into 1 initial load + N Commit refreshes. The Commit refreshes are needed for correctness — subsequent resolves must see prior resolve outcomes in the baseline.
...
Upload sessions are stored as JSON files keyed by `sha256(len(driveID):driveID:localPath)` — the length-prefix prevents delimiter collisions (e.g., `("a:", "b")` vs `("a", ":b")`). Stale sessions are auto-deleted via a lazy cleanup triggered on `Save()`, throttled to once per hour via double-checked locking (pre-check without goroutine, then lock inside goroutine). The `SessionStore` is shared between CLI `put` and sync engine. Session URLs are pre-authenticated — files are `0600` permissions. Corrupt session files return `ErrCorruptSession` (file is auto-deleted).
...
`PersistentPreRunE` runs two phases for all commands. Phase 1 (always): read Cobra flags into `CLIFlags`, build bootstrap logger, create `CLIContext` with `Flags` + `Logger`. Phase 2 (data commands only, skip via `skipConfigAnnotation`): load config, resolve drive, rebuild logger with config-based log level, populate `Cfg` + `Provider`. Auth commands get `CLIContext` with nil `Cfg`/`Provider` — they access `cc.Flags.ConfigPath` etc. No global flag variables anywhere.

### SessionProvider + Session replace DriveSession
`driveops.SessionProvider` caches `TokenSource`s by token file path via a mutex-protected map. Multiple drives sharing a token path share one `TokenSource`, preventing OAuth2 refresh token rotation races. `SessionProvider.Session(ctx, rd)` returns a `*driveops.Session` with `Meta` (30s timeout) + `Transfer` (no timeout) clients + `DriveID` + `Resolved`. Created once in `PersistentPreRunE`, stored in `CLIContext.Provider`, shared with the Orchestrator. Config is accessed via a shared `*config.Holder` (RWMutex-protected) — SIGHUP reload calls `holder.Update(newCfg)` once, updating config for both SessionProvider and Orchestrator. The sync command creates its own `SessionProvider` because it uses `skipConfigAnnotation`.

### config.Holder — single owner for mutable config
`config.Holder` wraps `*Config` + immutable config path behind an `RWMutex`. Both `SessionProvider` and `OrchestratorConfig` hold the same `*config.Holder` instance. On SIGHUP reload, one `holder.Update(newCfg)` call replaces the old two-line pattern (`o.cfg.Config = newCfg; o.cfg.Provider.UpdateConfig(newCfg)`). This prevents config divergence (forgetting one update) and narrows `SessionProvider.mu` to only protect the token cache.
...
### Accounts, drives, and config are separate concerns
Token resolution (`DriveTokenPath`) must NOT depend on config for shared drives. Previously `resolveSharedToken(cid, cfg)` scanned `cfg.Drives` to find the parent account — but during `drive add` (and many other paths) config wasn't loaded or was nil. Drive metadata files (`drive_*.json` at data root) store `account_canonical_id` for shared drives, and account profile files (`account_*.json` at data root) store user/org info. Resolution is pure file I/O — no config scanning. All data files use a flat layout with type prefixes (`token_`, `account_`, `drive_`, `state_`) directly in the data directory.
