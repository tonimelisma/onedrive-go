...
- **Reconciler F8/F9 unreachable through real engine (B-051).** `ListAllActiveItems` filters `is_deleted=0`, but `MarkDeleted` sets `is_deleted=1`. After delta processing, tombstoned items disappear from the reconciler's view. F8 (remote tombstone → local delete) and F9 (edit-delete conflict) work in mock-based reconciler tests but cannot fire through the real engine. E2E tests for this path are impossible without schema/query changes.
